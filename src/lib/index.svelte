<script lang="ts">
  ...
  export let toneSelector = true;
  export let searchPlaceholder = "Find the perfect emoji";
  ...
  let rawemojisData: any[] = [];
  let isLoadingEmojis = true;
  let recentlyUsed: IEmoji[] = [];
  let pickerRef: HTMLDivElement;
  let scrollRef: HTMLDivElement;
  let id = Math.random().toString(36).substring(2, 15);

  $: navHeight = showNav ? Math.floor(height / 7) : 0;

  onMount(() => {
    (async () => {
      const module = await import("./emojilib.json");
      rawemojisData = module.default;
      isLoadingEmojis = false;

      try {
        const stored = localStorage.getItem("hokkiemojipicker-recentlyused");
        recentlyUsed = stored ? JSON.parse(stored) : [];
      } catch (e) {
        recentlyUsed = [];
      }
    })();
  ...
  $: emojis = [
    ...
    (customEmojis?.emojis || []).length > 0
      ...
      : undefined,
    ...rawemojisData,
  ].filter((a): a is ICategory => a !== undefined);

  function handleSelect(emoji: IEmoji) {
    onEmojiSelect(emoji);
    
    if (emoji.char.startsWith("<")) return;

    let newRecentlyUsed = [...recentlyUsed];
    newRecentlyUsed = newRecentlyUsed.filter((b) => b.char !== emoji.char);
    newRecentlyUsed.push(emoji);
    newRecentlyUsed = newRecentlyUsed.slice(-20);
    
    recentlyUsed = newRecentlyUsed;
    localStorage.setItem("hokkiemojipicker-recentlyused", JSON.stringify(newRecentlyUsed));
  }

  const defaultIcons = {
    recentlyUsed: `<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M12 23a11 11 0 1 0 0-22 11 11 0 0 0 0 22Zm1-18a1 1 0 1 0-2 0v7c0 .27.1.52.3.7l3 3a1 1 0 0 0 1.4-1.4L13 11.58V5Z" clip-rule="evenodd"></path></svg>`,
    ...
    travel: `<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M15 4a1 1 0 0 0-.95 1.32l.9 2.68h-4.6l-.92-2.36A1 1 0 0 0 8.5 5H5a1 1 0 0 0 0 2h2.82l.5 1.27a1 1 0 0 0-.2.26L6.7 11.16a4.5 4.5 0 1 0 1.76.95l.78-1.45 1.83 4.7a1 1 0 1 0 1.86-.72L11.13 10h4.48l.55 1.65a4.5 4.5 0 1 0 1.9-.63L16.4 6h1.86c.41 0 .75.34.75.75V7a1 1 0 1 0 2 0v-.25A2.75 2.75 0 0 0 18.25 4H15Zm-9.3 9-1.08 2.03a1 1 0 0 0 1.76.94l1.09-2.01A2.5 2.5 0 1 1 5.7 13Zm11.13.64.72 2.18a1 1 0 0 0 1.9-.64l-.73-2.17a2.5 2.5 0 1 1-1.9.63Z" clip-rule="evenodd"></path></svg>`,
    objects: `<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path fill="currentColor" d="M10.41 3.59 11.6 2.4a2 2 0 0 1 2.82 0l1.3 1.3a1 1 0 0 0 .7.29h4.18a1.41 1.41 0 0 1 1 2.41L14.4 13.6a1.41 1.41 0 0 1-2.41-1V8.4l-3.11 3.12a2 2 0 0 0-.53 1.87L9.9 20H15a1 1 0 1 1 0 2H3a1 1 0 1 1 0-2h4.86L6.4 13.86a4 4 0 0 1 1.06-3.75L10.8 6.8l-.38-.38a2 2 0 0 1 0-2.82Z"></path><path fill="currentColor" d="M16.99 12.43c-.21.2-.2.55.06.7a3 3 0 0 0 4.08-4.08c-.15-.26-.5-.27-.7-.06l-3.44 3.44Z"></path></svg>`,
    symbols: `<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path fill="currentColor" d="M12.47 21.73a.92.92 0 0 1-.94 0C9.43 20.48 1 15.09 1 8.75A5.75 5.75 0 0 1 6.75 3c2.34 0 3.88.9 5.25 2.26A6.98 6.98 0 0 1 17.25 3 5.75 5.75 0 0 1 23 8.75c0 6.34-8.42 11.73-10.53 12.98Z"></path></svg>`,
    flags: `<svg aria-hidden="true" role="img" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24"><path fill="currentColor" d="M3 1a1 1 0 0 1 1 1v.82l8.67-1.45A2 2 0 0 1 15 3.35v1.47l5.67-.95A2 2 0 0 1 23 5.85v7.3a2 2 0 0 1-1.67 1.98l-9 1.5a2 2 0 0 1-1.78-.6c-.2-.21-.08-.54.18-.68a5.01 5.01 0 0 0 1.94-1.94c.18-.32-.1-.66-.46-.6L4 14.18V21a1 1 0 1 1-2 0V2a1 1 0 0 1 1-1Z"></path></svg>`
  };
  ...
  $: categoryData = {
    recentlyUsed: recentlyUsed.length > 0
      ? {
          name: "Recently Used",
          icon: defaultIcons.recentlyUsed,
        }
      : false,
    customServerIcons: (customEmojis.emojis || []).length > 0
      ? {
          name: customEmojis.serverName,
          icon: `<div class="relative rounded-full overflow-hidden"><img src="${customEmojis.serverIconURL}" class="w-full h-full object-cover" /></div>`,
        }
      : false,
    people: { name: "People", icon: defaultIcons.people, ...categories.people },
    nature: { name: "Nature", icon: defaultIcons.nature, ...categories.nature },
    food: { name: "Food", icon: defaultIcons.food, ...categories.food },
    activities: { name: "Activities", icon: defaultIcons.activities, ...categories.activities },
    travel: { name: "Travel", icon: defaultIcons.travel, ...categories.travel },
    objects: { name: "Objects", icon: defaultIcons.objects, ...categories.objects },
    symbols: { name: "Symbols", icon: defaultIcons.symbols, ...categories.symbols },
    flags: { name: "Flags", icon: defaultIcons.flags, ...categories.flags },
  } as any;
  
  const categoryOrder = ["recentlyUsed", "customServerIcons", "people", "nature", "food", "activities", "travel", "objects", "symbols", "flags"];
</script>

<div class="HOKKIEMOJIPICKER-emojiContainer">
  <div
    bind:this={pickerRef}
    id={"emojipicker-" + id}
    class={cn(
  ...
  >
    {#if showNav}
      <div
        class="HOKKIEMOJIPICKER-nav flex items-center gap-4 p-3 relative z-50"
        style:height="{navHeight}px"
        style:max-height="{navHeight}px"
        style:min-height="{navHeight}px"
      >
        <SearchBar {id} {searchPlaceholder} />
        {#if toneSelector}
          <SkinSelector {id} />
        {/if}
      </div>
    ...
    <div class="flex h-full">
      {#if showSidebar}
        <div class="HOKKIEMOJIPICKER-sidebar rounded-t-md bg-[#070709] flex p-2 gap-1 flex-col">
          {#each categoryOrder as categoryName (categoryName)}
            {#if categoryData[categoryName] !== false}
                 {#if categoryName === 'separator'}
                     <div class="w-full h-0 border-b-1 my-2"></div>
                 {:else}
                    <SidebarCategory
                        {id}
                        picker={pickerRef}
      ...
      <div
        style:height="{height - navHeight}px"
        style:max-height="{height - navHeight}px"
        style:min-height="{height - navHeight}px"
        class="HOKKIEMOJIPICKER-emojidisplaycontainer overflow-hidden flex flex-col w-full"
      >
        <div
          bind:this={scrollRef}
          style:flex-basis="fit-content"
          class="HOKKIEMOJIPICKER-emojidisplay overflow-y-scroll bg-[#131416] h-full w-full flex flex-wrap px-2 items-start justify-start justify-self-start gap-y-0-5"
        >
          {#if isLoadingEmojis}
             <div class="w-full h-full flex items-center justify-center">
                <div class="flex flex-col items-center gap-3">
                  <span class="text-transparent pointer-events-none select-none text-sm opacity-0">.</span>
                  <div class="animate-spin size-8 border-2 border-white/20 border-t-white/80 rounded-full"></div>
                  <span class="text-transparent pointer-events-none select-none text-sm opacity-0">.</span>
                </div>
              </div>
          {:else}
             {#each emojis as category (category.name)}
                <CategoryDisplay
                    {onEmojiMouseEnter}
                    {onEmojiMouseLeave}
                    ...
                    {category}
                    pickerId={id}
                    categoryInfo={categoryData[category.name]}
                />
             {/each}
          {/if}
        </div>
        {#if showFooter}
            <Footer {id} firstEmoji={emojis[0]?.emojis[0]} />
        {/if}
      </div>
    </div>
  </div>
